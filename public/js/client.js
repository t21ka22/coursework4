...
socket.on("new user", function (data) {
    username=data[data.length-1];
    addNewMessage({ user: username, message: `${username} has joined the chatroom` });
...
socket.on("user disconnected", function (userName) {
  ...
  addNewMessage({ user: userName, message: `${userName} has left the chatroom` });
});
